...
export function generateReceiptText(
  ...
  // Payments breakdown
  const payments: Array<{ method: string; amount: number; reference?: string }> = Array.isArray((sale as any).payments)
  ...
  const amountTendered = (sale as any).amountTendered;
  const change = (sale as any).change;
  const balanceDue = (sale as any).balanceDue;
  if (typeof amountTendered === "number" && amountTendered > 0) {
    text += line("Tendered", formatCurrency(amountTendered)) + "\n";
  }
  if (typeof change === "number" && change > 0) {
    text += line("Change", formatCurrency(change)) + "\n";
  }
