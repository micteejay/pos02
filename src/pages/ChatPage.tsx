...
import { MessageSquare, Hash, Users, Search, Send, Smile, Paperclip, MoreVertical, Plus, Phone, Video, Pin, X, ChevronDown, Settings, Bell, UserPlus, Trash2 } from "lucide-react";
...
const initialMessages: Message[] = [
  { id: "m1", sender: "Sarah Chen", avatar: "SC", time: "10:24 AM", text: "The Q4 inventory report is ready for review. I've shared it in the documents section.", channel: "general", reactions: [{ emoji: "👍", count: 3 }] },
  ...
  { id: "m4", sender: "You", avatar: "YO", time: "10:35 AM", text: "Great teamwork everyone. Let's sync up after lunch to finalize the approvals.", channel: "general", reactions: [{ emoji: "🎉", count: 2 }, { emoji: "✅", count: 1 }] },
  ...
  { id: "m8", sender: "Bob Tran", avatar: "BT", time: "9:30 AM", text: "Just closed a $12k deal with Metro Corp! 🎉", channel: "sales-team", reactions: [{ emoji: "🎉", count: 5 }, { emoji: "💰", count: 3 }] },
...
export default function ChatPage() {
  ...
  const channelMessages = useMemo(() => {
    ...
    if (searchText) {
      msgs = msgs.filter((m) => m.text.toLowerCase().includes(searchText.toLowerCase()) || m.sender.toLowerCase().includes(searchText.toLowerCase()));
    }
    return msgs;
  ...
  const createChannel = () => {
    ...
    channels.push({ id: newChannelName.toLowerCase().replace(/\s+/g, "-"), name: newChannelName.toLowerCase().replace(/\s+/g, "-"), type: "channel", unread: 0, members: 1, description: "New channel" });
    setActiveChannel(newChannelName.toLowerCase().replace(/\s+/g, "-"));
    setNewChannelName("");
          ...
          <div className="w-64 h-full border-r border-border bg-muted/30 flex flex-col shrink-0">
            <div className="p-3 border-b border-border flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setShowNewChannel(true)} className="p-1.5 rounded-md hover:bg-muted transition-colors">
                  <Plus className="w-4 h-4 text-muted-foreground" />
                </button>
                <button onClick={() => setShowMobileSidebar(false)} className="p-1.5 rounded-md hover:bg-muted transition-colors lg:hidden">
            ...
            <div className="p-2">
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-background border border-border text-xs">
                ...
                <input placeholder="Search channels..." className="bg-transparent outline-none w-full text-foreground placeholder:text-muted-foreground text-xs" />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-4">
              ...
              <div>
                ...
                {dms.map((dm) => (
                  <button
                    key={dm.id}
                    onClick={() => { setActiveChannel(dm.id); setShowMobileSidebar(false); }}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-14 border-b border-border px-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <button onClick={() => setShowMobileSidebar(true)} className="lg:hidden p-1.5 rounded-md hover:bg-muted mr-1">
              ...
              <div>
                ...
                {activeChannelData?.type === "channel" && (
                  <p className="text-[10px] text-muted-foreground">{activeChannelData.members} members · {activeChannelData.description}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2 text-muted-foreground">
              {activeChannelData?.type === "dm" && (
                <>
                  <button className="p-1.5 rounded-md hover:bg-muted transition-colors"><Phone className="w-4 h-4" /></button>
                  <button className="p-1.5 rounded-md hover:bg-muted transition-colors"><Video className="w-4 h-4" /></button>
                </>
              )}
              <button onClick={() => setShowSearch(!showSearch)} className="p-1.5 rounded-md hover:bg-muted transition-colors"><Search className="w-4 h-4" /></button>
              <button className="p-1.5 rounded-md hover:bg-muted transition-colors"><Users className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          {showSearch && (
            <div className="px-4 py-2 border-b border-border animate-fade-in">
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted text-sm">
                ...
                />
                <button onClick={() => { setShowSearch(false); setSearchText(""); }} className="p-0.5"><X className="w-4 h-4 text-muted-foreground" /></button>
              </div>
            </div>
          )}
                ...
                <div key={msg.id} className={`flex gap-3 group ${msg.sender === "You" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`max-w-[70%] ${msg.sender === "You" ? "text-right" : ""}`}>
                    ...
                    <div className="relative">
                      ...
                      {/* Actions */}
                      <div className={`absolute top-0 ${msg.sender === "You" ? "-left-20" : "-right-20"} hidden group-hover:flex items-center gap-0.5 bg-card border border-border rounded-lg p-0.5 shadow-md`}>
                        ...
                        {msg.sender === "You" && <button onClick={() => deleteMessage(msg.id)} className="p-1 rounded hover:bg-muted"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>}
                      </div>
                      {/* Emoji Picker */}
                      {showEmojiPicker === msg.id && (
                        <div className={`absolute top-8 ${msg.sender === "You" ? "right-0" : "left-0"} bg-card border border-border rounded-lg p-2 shadow-lg z-10 flex gap-1 animate-fade-in`}>
                          {emojiList.map((e) => (
                            <button key={e} onClick={() => addReaction(msg.id, e)} className="text-lg hover:scale-125 transition-transform p-0.5">
                              {e}
                            </button>
                          ))}
                        </div>
                    ...
                    {msg.reactions && msg.reactions.length > 0 && (
                      <div className={`flex gap-1.5 mt-1.5 ${msg.sender === "You" ? "justify-end" : ""}`}>
                        {msg.reactions.map((r) => (
                          <button key={r.emoji} onClick={() => addReaction(msg.id, r.emoji)} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-muted text-xs hover:bg-muted/80 transition-colors">
                            <span>{r.emoji}</span>
          ...
          <div className="p-3 border-t border-border">
            <div className="flex items-center gap-2 bg-muted rounded-xl px-4 py-3">
              ...
              />
              <Smile className="w-4 h-4 text-muted-foreground cursor-pointer hover:text-foreground transition-colors shrink-0" />
              <button onClick={sendMessage} disabled={!inputText.trim()} className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center hover:opacity-90 transition-opacity disabled:opacity-40 shrink-0">
        ...
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowNewChannel(false)}>
          <div className="glass-card rounded-2xl p-6 max-w-sm w-full animate-fade-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setShowNewChannel(false)} className="flex-1 py-2 rounded-lg border border-border text-sm font-medium text-foreground hover:bg-muted transition-colors">Cancel</button>
              <button onClick={createChannel} disabled={!newChannelName.trim()} className="flex-1 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors">Create</button>
            </div>
          </div>
        </div>
      )}
