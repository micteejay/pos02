...
interface CartItem {
  product: Product; qty: number; discount: number;
}
...
export default function POSPage() {
  ...
  return (
    <AppLayout>
          ...
          <div className="p-4 border-b border-border space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                {heldOrders.length > 0 && (
                  <button onClick={() => setShowHeld(!showHeld)} className="text-xs text-warning font-medium px-2 py-1 rounded-lg bg-warning/10 hover:bg-warning/20">
                    {heldOrders.length} held
                  </button>
                )}
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <div className="w-2 h-2 rounded-full bg-success animate-pulse" />Store: Main HQ
                </div>
              </div>
            </div>
            {showHeld && (
                ...
                return (
                  <button key={p.id} onClick={() => addToCart(p)} disabled={p.stock === 0}
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${p.stock < 15 ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success"}`}>{p.stock} left</span>
                    </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                ...
                <p className="text-sm">Cart is empty</p><p className="text-xs mt-1">Click products to add them</p>
              </div>
            ) : cart.map((item) => (
              <div key={item.product.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 group">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => updateQty(item.product.id, -1)} className="w-6 h-6 rounded bg-background border border-border flex items-center justify-center hover:bg-muted"><Minus className="w-3 h-3" /></button>
                  <span className="w-8 text-center text-sm font-semibold text-foreground">{item.qty}</span>
                  <button onClick={() => updateQty(item.product.id, 1)} className="w-6 h-6 rounded bg-background border border-border flex items-center justify-center hover:bg-muted"><Plus className="w-3 h-3" /></button>
                </div>
                <p className="text-sm font-semibold text-foreground w-16 text-right">{formatCurrency(item.product.price * item.qty)}</p>
                <button onClick={() => removeFromCart(item.product.id)} className="p-1 rounded hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-3.5 h-3.5 text-destructive" /></button>
              </div>
            ))}
          </div>
          {cart.length > 0 && (
      ...
      {/* Sale Complete Modal with Receipt */}
      {completedSale && (
          ...
          <div className="glass-card rounded-2xl p-6 max-w-sm w-full animate-fade-in max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 border border-border rounded-lg p-4 font-mono text-[10px] bg-card">
              ...
              <div className="flex justify-between font-bold text-foreground text-xs">
                <span>TOTAL</span><span>{formatCurrency(completedSale.total)}</span>
              </div>
