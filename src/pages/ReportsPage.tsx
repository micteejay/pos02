...
export default function ReportsPage() {
  ...
  const warehouseUtil = useMemo(() => {
    return warehouses.map(w => {
      const items = inventory.filter(i => i.warehouse === w.name).reduce((s, i) => s + i.qty, 0);
      const utilization = w.capacity > 0 ? Math.round((items / w.capacity) * 100) : 0;
      return {
        name: w.name,
  ...
  const printReport = async () => {
    ...
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${settings.appName} - ${reportType} Report</title>${reportStyles}</head><body>${bodyContent}<div class="footer">Generated by ${settings.appName} · ${new Date().toLocaleString()} · Printed by ${user?.name || "System"}</div></body></html>`;

    await printHtmlString(fullHtml, `${settings.appName} - ${reportType} Report`);
