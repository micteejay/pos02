      ...
      <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-5">
        {navSections.map((section) => (
          <div key={section.title}>
            {!collapsed && (
              <p className="text-[10px] font-semibold uppercase tracking-widest text-sidebar-foreground/50 px-3 mb-1.5">{section.title}</p>
            )}
                  ...
                  <Link key={item.path} to={item.path} onClick={onNavigate}
                    ...
                    {!collapsed && (
                      <>
                        ...
                        {item.badge && <span className="ml-auto bg-primary text-primary-foreground text-[10px] font-medium px-1.5 py-0.5 rounded-full min-w-[18px] text-center">{item.badge}</span>}
                      </>
                    )}
                    {collapsed && item.badge && <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-primary" />}
                  </Link>
                );
              })}
            </div>
